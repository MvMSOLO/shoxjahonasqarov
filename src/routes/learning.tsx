...
import { Play, Clock, BookOpen, Star, CheckCircle2 } from "lucide-react";
...
type Course = {
  ...
  level: "beginner" | "mid" | "advanced"; status: "active" | "done";
  progress: number; duration: string; lessons: number; rating: number;
  date: string; gradient: string; emoji: string;
};
      ...
      <div className="glass rounded-3xl p-4 shadow-card md:p-6">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 sm:gap-3">
          <div className="rounded-xl border border-border bg-card p-2.5 text-center sm:p-3">
            <div className="text-xl font-bold text-primary sm:text-2xl">{COURSES.filter(c => c.status === "active").length}</div>
            <div className="text-[10px] text-muted-foreground sm:text-xs">Faol kurslar</div>
          </div>
          <div className="rounded-xl border border-border bg-card p-2.5 text-center sm:p-3">
            <div className="text-xl font-bold text-success sm:text-2xl">{COURSES.filter(c => c.status === "done").length}</div>
            <div className="text-[10px] text-muted-foreground sm:text-xs">Yakunlangan</div>
          </div>
          <div className="rounded-xl border border-border bg-card p-2.5 text-center sm:p-3">
            <div className="text-xl font-bold text-warning sm:text-2xl">{activeTodos.length}</div>
            <div className="text-[10px] text-muted-foreground sm:text-xs">Vazifalar</div>
          </div>
        </div>

        <div className="mt-4 inline-flex rounded-xl border border-border bg-card p-1">
          <TabBtn active={tab === "all"} onClick={() => setTab("all")}>
            Barcha kurslar ({visible.length})
          </TabBtn>
          <TabBtn active={tab === "tasks"} onClick={() => setTab("tasks")}>
            Vazifalarim ({activeTodos.length})
          </TabBtn>
        </div>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {visible.map((c, i) => (
            <motion.div
              ...
              className="group glass overflow-hidden rounded-2xl shadow-card transition-transform hover:-translate-y-1"
            >
              <div className={`relative h-24 bg-gradient-to-br ${c.gradient} p-4 sm:h-28 sm:p-5`}>
                ...
                <span className="absolute right-2.5 top-2.5 rounded-full bg-black/30 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-white backdrop-blur sm:right-3 sm:top-3 sm:text-[10px]">
                  {c.level === "beginner" ? "Boshlang'ich" : c.level === "mid" ? "O'rta" : "Yuqori"}
                </span>
                {c.status === "done" && (
                  <div className="absolute bottom-2.5 right-2.5 flex items-center gap-1 rounded-full bg-success/80 px-2 py-0.5 text-[9px] font-bold text-white backdrop-blur">
              ...
              <div className="p-3 sm:p-4">
                ...
                <div className="mt-2.5">
                  <div className="flex justify-between text-[11px] text-muted-foreground">
                    ...
                    <span className={cn(
                      "font-semibold",
                      c.progress === 100 ? "text-success" : "text-foreground"
                    )}>{c.progress}%</span>
                  </div>
                ...
                <button
                  ...
                  className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-xl bg-gradient-primary px-3 py-2 text-xs font-semibold text-primary-foreground shadow-glow transition-transform hover:scale-[1.02]"
                >
                  <Play className="h-3.5 w-3.5" />
                  {c.status === "done" ? "Qayta ko'rish" : "Davom etish"}
                </button>
              </div>
            </motion.div>
          ))}
        ...
        <div className="space-y-3">
          {activeTodos.length > 0 && (
            <div className="glass rounded-2xl p-4 shadow-card md:p-5">
              <h2 className="text-sm font-bold">Bajarilmagan vazifalar ({activeTodos.length})</h2>
                ...
                {activeTodos.map((t) => (
                  <label key={t.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-border p-3 transition-colors hover:bg-accent">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={(e) => { e.preventDefault(); removeTodo(t.id); toast("Vazifa o'chirildi"); }}
          ...
          {doneTodos.length > 0 && (
            <div className="glass rounded-2xl p-4 shadow-card md:p-5">
              <h2 className="text-sm font-bold text-muted-foreground">Bajarilgan ({doneTodos.length})</h2>
              <div className="mt-3 space-y-2">
                {doneTodos.map((t) => (
                  <label key={t.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-border/50 p-3 opacity-60 transition-colors hover:bg-accent/50">
                    <input type="checkbox" checked={t.done} onChange={() => toggleTodo(t.id)} className="h-4 w-4 accent-primary" />
                    <div className="min-w-0 flex-1">
          ...
          {todos.length === 0 && (
            <div className="glass rounded-2xl p-10 text-center shadow-card">
              ...
              <p className="mt-2 text-sm text-muted-foreground">Barcha vazifalar bajarildi!</p>
            </div>
          )}
        </div>
      )}
